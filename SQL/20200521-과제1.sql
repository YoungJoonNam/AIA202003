...
----39. OUTER JOIN, SELF JOIN을 사용하여 관리자가 없는 
--사원을 포함하여 사원번호를 기준으로 내림차순 정렬하여 출력하시오.
----
select e.ename,e.empno,m.ename
from emp e, emp m
where e.mgr = m.empno(+)
...
select e.ename,e.deptno,ee.ename
from emp e, emp ee
where e.ename = 'SCOTT'  
...
----42. SELF JOIN 을 사용하여 관리자보다 먼저 입사한 모든 
