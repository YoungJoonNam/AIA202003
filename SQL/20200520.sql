...
select sal,mod(sal,3) as "%3"
from emp
;

select sal,mod(sal,3) as sal
from emp
;
...
select trunc(1282.23,1)
from dual
;

select trunc(1282.23,-2)
from dual
;
...
trunc(to_date('20201225','YYYYMMDD') - sysdate) as test
from dual
;
...
select deptno, avg(sal), count(*), count(comm),sum(comm),avg(comm)
from emp
group by deptno
having avg(sal) <= 2000
;
...
select job, count(*) as "직급별 인원",
                ...
                nvl(sum(comm),0) as "직급별 수령 커미션의 총합",
